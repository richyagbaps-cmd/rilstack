...
const CARDS: WidgetCard[] = [
  ...
  },
  {
    type: "ril-snack",
    title: "Ril-Snack",
    icon: <svg width="40" height="40" viewBox="0 0 40 40" fill="none"><circle cx="20" cy="20" r="20" fill="#2c3e5f"/><rect x="10" y="10" width="20" height="20" rx="6" fill="#4A8B6E"/><circle cx="30" cy="15" r="4" fill="#FFD700"/></svg>,
...
export default function SavingsInvestmentsCarousel() {
  ...
  const getCardIndex = (offset: number) => (active + offset + CARDS.length) % CARDS.length;
  return (
    <>
      <div
      ...
      >
        <div className="relative flex flex-row items-center justify-center w-full h-36">
          ...
          <button
            ...
            style={{ minWidth: 45, minHeight: 45, transform: 'translateY(9px) scale(0.85)' }}
            aria-label="Previous"
            onClick={() => { setActive((prev) => (prev - 1 + CARDS.length) % CARDS.length); setPaused(true); setTimeout(() => setPaused(false), 4000); }}
          >
            <div className="mb-1" style={{ fontSize: 20 }}>{CARDS[getCardIndex(-1)].icon}</div>
            <span className="text-xs font-semibold text-[#2c3e5f]">{CARDS[getCardIndex(-1)].title}</span>
          </button>
          ...
          >
            <div className="mb-2 drop-shadow-lg animate-bounce-slow" style={{ fontSize: 40 }}>{CARDS[active].icon}</div>
            <span className="text-lg font-extrabold text-center text-[#2c3e5f] tracking-wide drop-shadow">{CARDS[active].title}</span>
          </button>
          ...
          <button
            ...
            style={{ minWidth: 45, minHeight: 45, transform: 'translateY(9px) scale(0.85)' }}
            aria-label="Next"
            onClick={() => { setActive((prev) => (prev + 1) % CARDS.length); setPaused(true); setTimeout(() => setPaused(false), 4000); }}
          >
            <div className="mb-1" style={{ fontSize: 20 }}>{CARDS[getCardIndex(1)].icon}</div>
            <span className="text-xs font-semibold text-[#2c3e5f]">{CARDS[getCardIndex(1)].title}</span>
          </button>
      ...
      <BudgetSaveModal open={showBudgetSaveModal} onClose={() => setShowBudgetSaveModal(false)} />
      <style jsx global>{`
  ...
  return (
    <div className="flex flex-col items-center w-full">
      <div className="mb-2 flex items-center gap-2">
        ...
        <span className="text-xs bg-green-100 text-green-700 rounded px-2 py-0.5">Preview</span>
      </div>
      <div className="w-full flex flex-col items-center">
        ...
        <p className="text-center text-sm text-gray-700 dark:text-gray-200 mb-2">Eat the money, grow the snack!<br/>Arrow keys or on-screen controls.</p>
        <span className="inline-block text-xs text-blue-600 underline cursor-pointer">Tap to play full game</span>
      </div>
    </div>
  );
}

...
export default function SavingsGoalsSelector({
  demographics,
  onSelect,
  customGoals = [],
}: {
  ...
  const handleToggle = (goal: string) => {
    setSelected((sel) =>
      sel.includes(goal) ? sel.filter((g) => g !== goal) : [...sel, goal],
    );
  };
  ...
  return (
    <div className="w-full mt-6">
      <h2 className="text-lg font-bold mb-2 text-[#2c3e5f]">
      ...
      <div className="flex flex-col gap-2">
        {aiGoals.map((goal) => (
          <label key={goal.label} className="flex items-center gap-2">
            ...
            />
            <span>
              {goal.icon} <span className="font-semibold">{goal.label}</span>{" "}
              <span className="text-xs text-[#4A5B6E]">{goal.description}</span>
            </span>
          </label>
        ))}
        {customList.map((goal) => (
          <label key={goal} className="flex items-center gap-2">
            ...
            <span>
              ...
              <span className="text-xs text-[#4A5B6E]">(Custom)</span>
            </span>
          </label>
        ))}
        <div className="flex gap-2 mt-2">
          ...
          <button
            type="button"
            className="bg-[#00e096] text-white px-3 rounded"

...
export default function CreateGoalModal({
  ...
  const handleCreate = () => {
    ...
    onCreate({
      ...
      target,
      deadline,
      team: teamMode ? teamEmails : undefined,
      fundingSource,
    });
    onClose();
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-0 w-full max-w-md flex flex-col items-center relative">
        ...
        <div className="p-8 w-full">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-xl font-bold text-[#2c3e5f]">
              Create Savings Goal
            </h2>
          </div>
          {step === 0 && (
            <>
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {aiSuggestions.map((s) => (
                    <button
                      key={s}
                      className="bg-[#f3f4fa] px-3 py-1 rounded-full text-[#2c3e5f] border border-[#e6eaf0]"
                      onClick={() => {
                        setName(s);
                        setStep(1);
                      }}
              ...
              <button
                className="mt-2 text-[#00e096] underline"
                onClick={() => setStep(1)}
              >
                Custom goal
              </button>
          ...
          {step === 1 && (
            <>
              ...
              >
                ...
                <option value="auto-transfer">
                  Automatic daily/weekly transfer
                </option>
              </select>
              <div className="flex items-center gap-2 mb-4">
                ...
                />
                <label htmlFor="team-mode" className="text-[#2c3e5f]">
                  Team savings goal
                </label>
              </div>
              {teamMode && (
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {teamEmails.map((email) => (
                      <span
                        key={email}
                        className="bg-[#e6f7e6] px-2 py-1 rounded-full text-xs text-[#2c3e5f]"
                      >
                        {email}
                      </span>
                    ))}
              ...
              <button
                className="w-full bg-[#00e096] text-white px-4 py-2 rounded font-semibold"

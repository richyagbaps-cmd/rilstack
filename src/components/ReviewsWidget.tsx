...
export default function ReviewsWidget() {
  ...
  return (
    <div className={`w-full max-w-xs mx-auto transition-all duration-500 ${minimised ? 'h-14 overflow-hidden' : ''}`}>
      <div className={`bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-md p-3 transition-all duration-500 ${minimised ? 'py-2' : ''}`}>
        <div className="flex items-center justify-between mb-1">
          <span className="font-bold text-base text-blue-700">Suggestions Box</span>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              aria-label={minimised ? "Expand suggestions box" : "Minimise suggestions box"}
        ...
        {!minimised && success && <div className="mb-2 text-green-600 text-sm">{success}</div>}
        {!minimised && showForm && (
          <form onSubmit={handleSubmit} className="mb-3">
            ...
            <div className="flex items-center mb-2">
              ...
              <span className="ml-2 text-sm">{rating > 0 ? `${rating}/5` : "Rate"}</span>
            </div>
            ...
            {reviews.slice(0, 5).map((r, i) => (
              <div key={i} className="py-2">
                ...
                <div className="flex items-center mb-1">
                  {[1,2,3,4,5].map((star) => (
                    <span key={star} className={`text-lg ${star <= r.rating ? "text-yellow-400" : "text-gray-200"}`}>★</span>
                  ))}
                </div>

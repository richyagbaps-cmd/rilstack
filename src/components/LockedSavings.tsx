...
export default function LockedSavings() {
  ...
  const onSubmit = async (data: {
    ...
    const newSaving = {
      ...
      interestRate:
        data.lockPeriod === "yearly"
          ? 7.2
          : data.lockPeriod === "monthly"
            ? 4.5
            : data.lockPeriod === "daily"
              ? 2.0
              : 0.5,
    };
  ...
  const getTotalInterest = () => {
    return lockedSavings
      .reduce((sum, s) => sum + (s.amount * s.interestRate) / 100, 0)
      .toFixed(2);
  };
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="text-blue-700 text-xs">
              Lock for 1 month, 4.5% interest
            </p>
          </div>
          <div>
            <p className="font-semibold text-blue-800">📊 Yearly</p>
            <p className="text-blue-700 text-xs">
              Lock for 1 year, 7.2% interest
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          {lockedSavings
            ...
            .map((saving) => (
              <div
                key={saving.id}
                className={`bg-gradient-to-r ${getPeriodColor(saving.lockPeriod)} text-white p-5 rounded-lg`}
              >
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <p className="text-xs opacity-90">
                      {getLockPeriodLabel(saving.lockPeriod)}
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-semibold">
                      {timeRemaining[saving.id] || "Calculating..."}
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-semibold">
                      ₦
                      {(
                        (saving.amount * saving.interestRate) /
                        100
                      ).toLocaleString()}
                    </p>
                  </div>
                ...
                <div className="mt-3 bg-white bg-opacity-20 backdrop-blur px-3 py-2 rounded text-xs">
                  <p>
                    Unlock Date:{" "}
                    {new Date(saving.unlockDate).toLocaleDateString("en-US", {
                      weekday: "short",
                      year: "numeric",
                      month: "short",
                      day: "numeric",
                  ...
                  <p className="mt-1">
                    ...
                    {(
                      saving.amount +
                      (saving.amount * saving.interestRate) / 100
                    ).toLocaleString()}{" "}
                    total
                  </p>
                </div>
          ...
          <div className="space-y-3">
            {lockedSavings
              ...
              .map((saving) => (
                <div
                  key={saving.id}
                  className="p-4 border-2 border-green-300 bg-green-50 rounded-lg flex justify-between items-center"
                >
                  <div>
                  ...
                  <div className="text-right">
                    <p className="text-lg font-semibold text-gray-700 mb-1">
                      Principal: ₦{saving.amount.toLocaleString()}
                    </p>
                    <p className="text-sm text-green-600 mb-2">
                      Interest: ₦
                      {saving.interestEarned?.toLocaleString() || "0"}
                    </p>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="bg-yellow-50 border border-yellow-200 p-3 rounded text-sm text-yellow-800">
            <p>
              <strong>⚠️ Important:</strong> Once locked, you cannot withdraw
              funds until the lock period expires. Interest is automatically
              added to your account when the lock period ends. Choose your lock
              period carefully!
            </p>
          </div>

...
export default function MagicWandAI({ onShuffle, onUndo, disabled }: { onShuffle: () => void; onUndo: () => void; disabled?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        ...
        onClick={() => { setShuffled(true); onShuffle(); }}
      ...
      {shuffled && (
        <button
          ...
          onClick={() => { setShuffled(false); onUndo(); }}
        >Undo</button>
      )}
    </div>
  );

...
const BUDGET_TYPES = [
  {
    ...
    icon: "📊",
    description:
      "AI assigns 50% to Needs, 30% to Wants, 20% to Savings. Simple, effective, and automated.",
    preview: [
  ...
  },
  {
    ...
    icon: "🧮",
    description:
      "Every naira is assigned to a category. AI suggests splits based on your income and profile.",
    preview: [
  ...
  },
  {
    ...
    icon: "⚙️",
    description:
      "Manually assign percentages or fixed amounts to each category. Full control for advanced users.",
    preview: [{ label: "Custom", percent: 100, color: "bg-[#4A5B6E]" }],
  },
];
    ...
    <div className="min-h-screen bg-[#F8F9FC] flex flex-col items-center justify-center p-4 md:p-8">
      <div className="mx-auto mb-10 max-w-2xl text-center">
        ...
        <p className="text-base text-[#4A5B6E] md:text-lg">
          Pick a budgeting method that fits your lifestyle. You can change this
          later.
        </p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-4xl">
        {BUDGET_TYPES.map((type) => (
          <div
            key={type.key}
            onClick={() => {
              setSelected(type.key as any);
              onTypeSelect(type.key as any);
            }}
          ...
          >
            <div className="flex flex-col items-center mb-4">
              ...
              <h2 className="text-xl font-bold text-[#2c3e5f] mb-2">
                {type.label}
              </h2>
              ...
              <div className="w-full flex gap-1 mb-2">
                {type.preview.map((p, i) => (
                  <div
                    key={i}
                    className={`h-4 rounded-full ${p.color}`}

...
export const formatNaira = (amount: number) => `N${Math.round(amount).toLocaleString()}`;
...
export const loadAutomationSettings = (): AutomationSettings => {
  ...
  try {
    ...
    return {
      ...
      ...parsedValue,
      autosavePercent: clampNumber(Number(parsedValue.autosavePercent ?? DEFAULT_AUTOMATION_SETTINGS.autosavePercent), 1, 40),
      paydaySweepDay: clampNumber(Number(parsedValue.paydaySweepDay ?? DEFAULT_AUTOMATION_SETTINGS.paydaySweepDay), 1, 31),
      emergencyBufferTarget: Math.max(5000, Number(parsedValue.emergencyBufferTarget ?? DEFAULT_AUTOMATION_SETTINGS.emergencyBufferTarget)),
    };
  } catch {
    return DEFAULT_AUTOMATION_SETTINGS;
  }
...
export const saveAutomationSettings = (settings: AutomationSettings) => {
  ...
  window.localStorage.setItem(AUTOMATION_SETTINGS_KEY, JSON.stringify(settings));
};
...
export const estimateMonthlyAutoSave = (income: number, settings: AutomationSettings) => {
  const baseAutoSave = settings.autosaveEnabled ? income * (settings.autosavePercent / 100) : 0;
  const roundUps = settings.roundUpsEnabled ? getRoundUpAverage(settings.roundUpMode) : 0;
...
export const getNextSweepLabel = (paydaySweepDay: number) => {
  ...
  const sweepDate = new Date(today.getFullYear(), today.getMonth(), paydaySweepDay);

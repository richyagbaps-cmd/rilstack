...
function BudgetSummaryPage() {
  ...
  if (type === "502030" || type === "zero-based") {
    ...
    try {
      ...
      allocations = Object.entries(parsed).map(([label, amount]) => ({ label, amount }));
    } catch {}
  }

  const savingsAmount = Number(type === "custom" ? allocations?.find(a => a.label.toLowerCase().includes("savings"))?.amount : savings);
  ...
  const hasSavings = allocations && allocations.some(a => a.label.toLowerCase().includes("savings"));
  // Helper to check if safe lock is valid
  const safeLockValid = !safeLock || (finishDate && daysBetween(startDate, finishDate) >= 10);

  // Show goals selector if savings pocket exists and not yet selected
  useEffect(() => {
    if (allocations && allocations.some(a => a.label.toLowerCase().includes("savings")) && selectedGoals.length === 0) {
      setShowGoals(true);
    }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#f8f9fc] p-8">
      <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full flex flex-col items-center">
        <h1 className="text-2xl font-bold mb-6 text-[#2c3e5f]">Your Budget Allocation</h1>
          ...
          {allocations && allocations.map(a => (
            <li key={a.label} className="mb-2 flex flex-col w-full">
              ...
              <div className="mt-1 flex items-center gap-2">
                ...
                <select
                  value={getFrequency(a.label, typeof a.amount === 'number' ? a.amount : Number(a.amount) || 0)}
                  onChange={e => handleFrequencyChange(a.label, e.target.value)}
        ...
        <div className="w-full mt-6 flex flex-col gap-3">
          ...
          <label className="flex flex-col">
            Finish Date
            <input type="date" value={finishDate} onChange={e => setFinishDate(e.target.value)} className="mt-1 p-2 border rounded" min={startDate} />
          </label>
        </div>
        {showInterest && (
          <div className="mt-4 w-full bg-[#eaf2fa] rounded-xl p-4 text-center">
            <div className="font-semibold text-[#2c3e5f]">Estimated Interest on Savings</div>
            <div className="text-lg font-bold text-[#00e096]">₦{interest.toLocaleString()}</div>
            <div className="text-xs text-[#4A5B6E] mt-1">({days} days at 0.05% daily)</div>
            <div className="text-xs text-[#4A5B6E]">Time Remaining: {days} days</div>
          </div>
        )}
        ...
        {hasSavings && (
          <div className="w-full mt-4 flex flex-col gap-2">
            <label className="flex items-center gap-2">
              ...
              />
              <span className="text-xs text-[#4A5B6E]">Enable Safe Lock (money only available after finish date, minimum 10 days)</span>
            </label>
            {safeLock && finishDate && daysBetween(startDate, finishDate) < 10 && (
              <span className="text-xs text-red-600">Safe lock requires at least 10 days between start and finish date.</span>
            )}
          </div>
        )}
        ...
        <button
          ...
          disabled={!agreed || pinConfirmed || (showGoals && selectedGoals.length === 0) || !safeLockValid}

...
export default function RelaxedBudgetPage() {
  ...
  const handleAddPocket = () => {
    setPockets([
      ...pockets,
      { key: `custom${Date.now()}`, label: "New Expense", amount: 0 },
    ]);
  };
  ...
  const handleWithdraw = (idx: number) => {
    alert(
      `Withdrawing from ${pockets[idx].label}. A 3.5% fee will be applied.`,
    );
  };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#f8f9fc] p-8">
      <div className="bg-white rounded-2xl shadow-xl p-8 max-w-2xl w-full flex flex-col items-center">
        ...
        <p className="mb-8 text-[#4A5B6E] text-center max-w-lg">
          Your money is divided into spending pockets for each category. In
          relaxed mode, you can withdraw from any pocket at any time, but a 3.5%
          fee applies to any amount withdrawn before the end date.
        </p>
        {!enabled && (
        ...
        {enabled && (
          <div className="w-full text-green-600 text-center font-semibold mb-2">
            Budget is now enabled. You can withdraw from any pocket (3.5% fee
            applies).
          </div>
        )}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full mb-8">
          {pockets.map((pocket, idx) => (
            <div
              key={pocket.key}
              className="bg-[#fffbe6] rounded-xl p-4 flex flex-col items-center shadow relative"
            >
              <input
                type="text"
        ...
        <div className="w-full text-xs text-[#7a869a] text-center">
          <strong>Note:</strong> You can withdraw from any pocket, but a 3.5%
          fee applies before the end date.
        </div>
      </div>
      <PinConfirmModal

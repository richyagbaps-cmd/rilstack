...
const TBILLS = [
  {
    ...
    discountRate: "7.2%",
    interestCalc:
      "Interest is the difference between your bid price and the par value (₦100). Paid upfront.",
  },
  {
    id: "tbill-182",
    name: "182-day T-bill",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md relative">
        ...
        <ul className="space-y-2 text-sm">
          <li>
            <span className="font-semibold">Tenor:</span> {tbill.tenor} days
          </li>
          <li>
          ...
          <li>
            <span className="font-semibold">Minimum Unit:</span> {tbill.minUnit}
          </li>
          <li>
            <span className="font-semibold">Auction Closing Date:</span>{" "}
            {tbill.auctionClose}
          </li>
          <li>
            <span className="font-semibold">Issue Date:</span> {tbill.issueDate}
          </li>
          <li>
          ...
          <li>
            <span className="font-semibold">Interest Calculation:</span>{" "}
            {tbill.interestCalc}
          </li>
        </ul>
      </div>
...
function BondAuctionDetailModal({
  bond,
  onClose,
}: {
  bond: PrimaryBond;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md relative">
        ...
        <ul className="space-y-2 text-sm">
          <li>
            <span className="font-semibold">Coupon Rate:</span> {bond.coupon}
          </li>
          <li>
          ...
          <li>
            <span className="font-semibold">Issue Date:</span> {bond.issueDate}
          </li>
          <li>
            <span className="font-semibold">Maturity Date:</span>{" "}
            {bond.maturityDate}
...
function MutualFundDetailModal({
  fund,
  onClose,
}: {
  fund: MutualFund;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md relative">
        ...
        <ul className="space-y-2 text-sm mb-4">
          <li>
            <span className="font-semibold">Manager:</span> {fund.manager}
          </li>
          <li>
            <span className="font-semibold">1-Year Return:</span>{" "}
            {fund.oneYearReturn}
          </li>
          <li>
            <span className="font-semibold">Risk Level:</span> {fund.risk}
          </li>
          <li>
          ...
          <li>
            <span className="font-semibold">Min Subscription:</span>{" "}
            {fund.minSubscription}
          </li>
          <li>
            <span className="font-semibold">Portfolio:</span> {fund.portfolio}
          </li>
          <li>
            <span className="font-semibold">Fact Sheet:</span>{" "}
            <a
        ...
        <div className="mb-2 font-semibold text-sm">
          Historical NAV (chart placeholder):
        </div>
        <div className="w-full h-24 bg-slate-100 rounded flex items-end gap-1 p-2">
          {fund.navHistory.map((nav, i) => (
            <div
              key={i}
              style={{
                height: `${10 + (nav - Math.min(...fund.navHistory))}px`,
              }}
...
function SubscribeAuctionModal({
  product,
  unit,
  deadline,
  onClose,
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md relative">
        ...
        {deadlinePassed && (
          <div className="mb-2 text-red-600 font-semibold">
            Auction deadline has passed. Next auction: [show next date]
          </div>
        )}
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="text-xs text-slate-500 mt-1">
              Must be a multiple of ₦{unit.toLocaleString()}
            </div>
            {amount > USER_BALANCE && (
          ...
          <div>
            <label className="block text-sm font-semibold mb-1">
              Bid Yield (%) <span className="text-slate-400">(optional)</span>
            </label>
            <input
              type="number"
              step="0.01"
...
function BuySecondaryBondModal({
  bond,
  onClose,
}: {
  bond: SecondaryBond;
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md relative">
        ...
        {!marketOpen && (
          <div className="mb-2 text-yellow-700 font-semibold">
            Market is closed. You can place this order as "Good for Next Open".
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="text-sm text-slate-700 bg-slate-50 rounded p-3">
            <div>
              Principal: ₦
              {((nominal * price) / 100).toLocaleString(undefined, {
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-xs relative z-10">
              ...
              <div className="text-sm mb-2">
                Buy order submitted to backend!
              </div>
              <div className="flex gap-2 mt-4">
                <button
                  className="flex-1 rounded bg-[#2c3e5f] py-2 font-semibold text-white"
                  onClick={onClose}
                >
                  Close
                </button>
...
function SecondaryBondReviewModal({
  bond,
  quantity,
  price,
  accrued,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md relative">
        ...
        <ul className="text-sm mb-3 space-y-1">
          ...
          <li>
            <b>Accrued Interest:</b> ₦{accrued.toLocaleString()}
          </li>
          <li>
            <b>Fees:</b> ₦{fees.toLocaleString()}
          </li>
          <li>
            <b>Total Debit:</b> ₦
            {total.toLocaleString(undefined, { maximumFractionDigits: 2 })}
          ...
          <li>
            <b>Settlement Date:</b> {settlement}
          </li>
        </ul>
        <RiskCheckbox checked={risk} onChange={setRisk} />
        <button
...
// Mutual Fund Review
function MutualFundReviewModal({
  fund,
  amount,
  units,
  navDateStr,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md relative">
        ...
        <ul className="text-sm mb-3 space-y-1">
          ...
          <li>
            <b>NAV Date:</b> {navDateStr}
          </li>
          <li>
            <b>Redemption Policy:</b> {redemption}
          </li>
        </ul>
        <RiskCheckbox checked={risk} onChange={setRisk} />
        <button
...
const TBillBondAuctions = ({
  addPending,
}: {
  addPending: (order: any) => void;
}) => {
  ...
  return (
    <div className="space-y-4">
          ...
          <li key={tbill.id} className="py-3 flex flex-col gap-1">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  className="text-xs text-[#2c3e5f] underline font-semibold"
                  onClick={() => setSelectedTBill(tbill)}
                >
                  Details
                </button>
                <button
            ...
            <span className="text-xs text-slate-500">
              Tenor: {tbill.tenor} days
            </span>
            <span className="text-xs text-slate-500">
              Expected Yield: {tbill.expectedYield}
            </span>
            <span className="text-xs text-slate-500">
              Min Unit: {tbill.minUnit}
            </span>
            <span className="text-xs text-slate-500">
              Auction Closes: {tbill.auctionClose}
            </span>
          </li>
        ))}
      </ul>
      ...
      {review && (
        <TBillReviewModal
          ...
          onConfirm={() => {
            setReview(null);
            setFeedback(
              "Subscription received. Allocation will be confirmed after auction date.",
            );
            addPending({
      ...
      {feedback && (
        <div className="bg-blue-50 border border-blue-200 text-blue-800 rounded p-4 mt-4">
          {feedback}
        </div>
      )}
    </div>
      ...
      <ul className="divide-y divide-slate-200">
        {PRIMARY_BONDS.map((bond) => (
          <li key={bond.id} className="py-3 flex flex-col gap-1">
            ...
            <span className="text-xs text-slate-500">
              Offer Amount: {bond.offerAmount}
            </span>
            <span className="text-xs text-slate-500">
              Auction Closes: {bond.auctionClose}
            </span>
          </li>
        ))}
      </ul>
      {selectedBond && (
        <BondAuctionDetailModal
          bond={selectedBond}
      ...
      <ul className="divide-y divide-slate-200">
        {SECONDARY_BONDS.map((bond: SecondaryBond) => (
          <li key={bond.id} className="py-3 flex flex-col gap-1">
            ...
            <span className="text-xs text-slate-500">
              Bid: {bond.bid} | Ask: {bond.ask}
            </span>
            <span className="text-xs text-slate-500">
              YTM: {bond.ytm} | Accrued Interest: {bond.accruedInterest}
            </span>
            <span className="text-xs text-slate-500">
              Settlement: {bond.settlement}
            </span>
            <div className="flex gap-2 mt-1">
              ...
              <button className="px-3 py-1 rounded bg-red-500 text-white text-xs font-semibold hover:bg-red-600">
                Sell
              </button>
            </div>
          </li>
      ...
      <ul className="divide-y divide-slate-200">
        {MUTUAL_FUNDS.map((fund) => (
          <li key={fund.id} className="py-3 flex flex-col gap-1">
            ...
            <span className="text-xs text-slate-500">
              1-Year Return: {fund.oneYearReturn}
            </span>
            ...
            <span className="text-xs text-slate-500">
              Min Subscription: {fund.minSubscription}
            </span>
          </li>
        ))}
      </ul>
...
const usePendingOrders = () => {
  ...
  return {
    pending,
    addPending: (order: any) => setPending((p) => [...p, order]),
    setPending,
  };
};
...
export default function InvestmentsPage() {
  ...
  const TABS = [
    {
      key: "auctions",
      label: "T-bill/Bond Auctions",
      component: <TBillBondAuctions addPending={pendingState.addPending} />,
    },
    {
      key: "secondary",
      label: "Secondary Market",
      component: <SecondaryMarketBonds addPending={pendingState.addPending} />,
    },
    {
      key: "mutual",
      label: "Mutual Funds",
      component: <MutualFunds addPending={pendingState.addPending} />,
    },
...
function MutualFundInvestModal({
  fund,
  onClose,
}: {
  fund: MutualFund;
  ...
  const [amount, setAmount] = useState(
    Number(fund.minSubscription.replace(/[^\d]/g, "")) || 10000,
  );
  ...
  const redemption =
    fund.risk === "Low" ? "T+1 (Next business day)" : "T+3 (3 business days)";

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    if (
      amount < Number(fund.minSubscription.replace(/[^\d]/g, "")) ||
      amount % 1000 !== 0
    ) {
      setError(
        `Minimum amount is ${fund.minSubscription}, in multiples of ₦1,000.`,
      );
      return;
    }
    setShowReview(true);
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            />
            <div className="text-xs text-slate-500 mt-1">
              Minimum {fund.minSubscription}, multiples of ₦1,000
            </div>
          </div>
          {mode === "sip" && (
            <div className="flex gap-2">
              <div>
                ...
                <select
                  value={sipFreq}
                  onChange={(e) =>
                    setSipFreq(e.target.value as "monthly" | "quarterly")
                  }
          ...
          <div>
            ...
            <div className="flex gap-4 mt-1">
              <label className="flex items-center gap-1">
                <input
                  type="radio"
                  checked={dividend === "growth"}
                  onChange={() => setDividend("growth")}
                />
              ...
              <label className="flex items-center gap-1">
                <input
                  type="radio"
                  checked={dividend === "idcw"}
                  onChange={() => setDividend("idcw")}
                />
          ...
          <div className="text-xs text-slate-700 bg-slate-50 rounded p-3">
            Order will be executed at next NAV ({navDateStr}){" "}
            {afterCutoff && (
              <span className="text-red-600">(after cut-off time)</span>
            )}
            .
          </div>
        </form>
        ...
        {showConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-xs relative z-10">
              <h4 className="text-lg font-bold mb-2">Order Placed</h4>
              <div className="text-sm mb-2">
                Investment order submitted to backend!
              </div>
              <div className="flex gap-2 mt-4">
                <button
                  className="flex-1 rounded bg-[#2c3e5f] py-2 font-semibold text-white"

...
export default function AdminPage() {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['#', 'Name', 'Email', 'Phone', 'KYC Level', 'KYC Status', 'Balance', 'Date Joined', 'Last Updated'];
        ...
        <div className="w-full max-w-sm">
          <div className="bg-white rounded-2xl shadow-lg p-8">
            <div className="text-center mb-8">
              ...
              <p className="text-[#4A5B6E] text-sm mt-1">Enter password to continue</p>
            </div>
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-[#1E2A3A]">Rilstack Admin</h1>
              <p className="text-xs text-[#4A5B6E]">User Management Dashboard</p>
            </div>
          </div>
          <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl p-5 border border-gray-100">
            ...
            <p className="text-lg font-bold mt-1">
              <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${storage === 'redis' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
        ...
        {users.length === 0 ? (
          <div className="bg-white rounded-xl border border-gray-100 p-12 text-center">
            ...
            <p className="text-[#4A5B6E] font-medium">No registered users yet</p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[#F8F9FC] border-b border-gray-100">
                    <th className="text-left px-4 py-3 font-semibold text-[#4A5B6E] whitespace-nowrap">#</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#4A5B6E] whitespace-nowrap">Name</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#4A5B6E] whitespace-nowrap">Email</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#4A5B6E] whitespace-nowrap">Phone</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#4A5B6E] whitespace-nowrap">KYC</th>
                    <th className="text-right px-4 py-3 font-semibold text-[#4A5B6E] whitespace-nowrap">Balance</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#4A5B6E] whitespace-nowrap">Joined</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#4A5B6E] whitespace-nowrap">Last Login</th>
                  </tr>
                </thead>
                <tbody>
                  {users.map((user, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-[#4A5B6E] whitespace-nowrap">{user.email || '—'}</td>
                      <td className="px-4 py-3 text-[#4A5B6E] whitespace-nowrap">{user.phone || '—'}</td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-medium ${getKycColor(user.kycLevel)}`}>
                          {getKycLabel(user.kycLevel)}
                        </span>
                      </td>

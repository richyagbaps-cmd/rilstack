  ...
  try {
    ...
    if (!PAYSTACK_SECRET_KEY && !OPAY_API_KEY) {
      return NextResponse.json(
        { error: "No payment API configured" },
        { status: 500 },
      );
    }
    ...
    const refId =
      `OPAY_${Date.now()}_${Math.random().toString(36).substring(7)}`.toUpperCase();

    const opayResponse = await fetch(
      `${OPAY_API_URL}/third/depositcode/generateStaticDepositCode`,
      {
        method: "POST",
        headers: {
